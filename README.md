...
- **푸르니 로그인 아이디/비밀번호** (purunicare.com에서 사용하는 것)
- **인터넷 연결**
...
3. 검은 창이 뜨면 안내에 따라 **아이디**와 **비밀번호**를 입력합니다
4. 자동으로 다운로드가 시작됩니다!
...
`실행.bat` 대신 직접 명령어를 입력할 수 있습니다:

```bash
node scraper.js --id=아이디 --pw="비밀번호" --class=반코드 --child=원아키
```

반코드와 원아키는 푸르니 알림장 페이지 URL에서 확인할 수 있습니다.
